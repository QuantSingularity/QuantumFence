...
class DroneTrack:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "avg_confidence":  round(float(np.mean(list(self.confidences))), 3)
...
class DroneDetector:
    ...
    def process_frame(
        ...
        now           = datetime.utcnow()
        valid_dets    = [d for d in raw_detections
    ...
    def _calculate_threat_score(
        self, track: DroneTrack, frame_shape: tuple
    ) -> float:
        score = 0.3
    ...
    def detect_swarm(self) -> bool:
        ...
        directions = [t.trajectory_direction for t in active
                      if t.trajectory_direction is not None]
        if len(directions) < 3:
            return False
